...
int16_t DjiEsc::tx_iq_refs_[8] = {0};

/** speed_pid_的输出是iqref, 那么其最后一个参数output_limit设置为参考电流的最大值 */
DjiEsc::DjiEsc(MotorType motor_type, EscId esc_id, FDCAN_HandleTypeDef *hfdcan)
    ...
    motor_type_(motor_type)
{
    ...
    uint8_t index = static_cast<uint8_t>(esc_id_) - 1;
    is_id_active_[index] = true;
}

/** 因为一次CAN通信能控制4个ID的电调, 所以可以在一个控制周期内, 先写入所有控制输出值, 最后再调用下面的SendAllCanTxData()统一控制; 分次发送有可能会覆盖前面的数据 */
void DjiEsc::SetTxIqRef(int16_t iq_tx_data)
{
    uint8_t index = static_cast<uint8_t>(esc_id_) - 1;
    tx_iq_refs_[index] = iq_tx_data;
}


void DjiEsc::SendAllCanTxData(FDCAN_HandleTypeDef *hfdcan)
{
    ...
    bool need_send_200 = is_id_active_[0] || is_id_active_[1] || is_id_active_[2] || is_id_active_[3];
    bool need_send_1FF = is_id_active_[4] || is_id_active_[5] || is_id_active_[6] || is_id_active_[7];
    ...
    if (need_send_200)
    {
        ...
        HAL_FDCAN_AddMessageToTxFifoQ(hfdcan, &pTxHeader, data_200);
    }
    else if (need_send_1FF)
    {
        ...
        HAL_FDCAN_AddMessageToTxFifoQ(hfdcan, &pTxHeader, data_1FF);
    }
}

/** CAN接收回调函数, 解析反馈数据; 注意, 解析得到的是减速器输出端的位置和速度 */
void DjiEsc::CanRxCallBack(uint32_t can_std_id, uint8_t *rx_data)
{
    if (rx_data == nullptr) return;
    
    uint32_t expected_can_id = 0x200 + static_cast<uint8_t>(esc_id_);
    if (can_std_id != expected_can_id) return;

    /** 1. 物理位置解析 */
    rotor_last_pos_one_round_rad_ = rotor_pos_one_round_rad_;
    uint16_t raw_pos = static_cast<uint16_t>((rx_data[0] << 8) | rx_data[1]);
    rotor_pos_one_round_rad_ = _2PI * static_cast<float>(raw_pos) / kRxDataToPositionDeg;
