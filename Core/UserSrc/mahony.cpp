 ...
 *   1. 磁力计全零检测 → 降级为 6 轴模式
 *   2. 加速度计/磁力计归一化
 *   3. 估计重力方向 (由四元数推导) 和地磁方向
 *   4. 计算估计方向与测量方向的叉积 (姿态误差)
 *   5. PI 修正陀螺仪角速度: g_corrected = g + Kp·e + Ki·∫e
 *   6. 四元数积分: q += 0.5·q⊗ω·dt
 *   7. 四元数归一化
 */
 ...
 *
 * 与 9 轴模式的区别: 不依赖磁力计，仅使用重力矢量修正 pitch/roll。
 * yaw 角没有绝对参考，会随时间漂移。
 *
 * 流程:
 *   1. 加速度计归一化 → 测量重力方向
 ...
 *
 * 原理: IEEE 754 浮点数的位操作近似 + 牛顿迭代法精化。
 * 魔数 0x5f3759df 提供一个良好的初始猜测，两次牛顿迭代达到足够精度。
 *
 * 替代 1.0f/sqrtf(x)，避免浮点除法指令开销，
 * 对 Cortex-M7 单精度 FPU 有显著性能提升。
 *
 * @param x 输入正数
 ...
 *
 * 采用惰性求值: 仅在姿态更新后才需要重新计算，由 is_angle_computed_ 标记控制。
 */
