...
HermiteInterp::HermiteInterp(float* ptr_xs, float* ptr_ys, float *ptr_dys, uint16_t num_xs)
{
    if (num_xs < 2 || ptr_xs == nullptr || ptr_ys == nullptr || ptr_dys == nullptr)
    {
    ...
    // minimum allowed spacing between adjacent xs
...
void HermiteInterp::Interp(float x_interp_interval)
{
    ...
    for (uint16_t i = 0; i < num_interp_; ++i)
    {
        ...
        {
            for (uint16_t j = 0; j < (uint16_t)(num_xs_ - 1); ++j)
            {
                if (x >= ptr_xs_[j] && x < ptr_xs_[j + 1])
                {
                    interval_index = j;
                    break;
                }
